...
def test_harvest_corpus_anti_patterns(tmp_path: Path) -> None:
    from minuscorrect.anti_cheat import harvest_corpus_anti_patterns, load_banned_anti_patterns

    corpus_file = tmp_path / "sec100p_corpus.py"
    corpus_content = """
# Real-world benchmark corpus example: MinusBrain sec100p
SEC100P_BENCHMARK = [
    (
        "sec_10k_p3_cash_flows",
    ...
    (
        "sec_10k_p7_balance_sheet",
        "Consolidated Balance Sheet",
        [
            ("Cash and cash equivalents", 29965.0, 28308.0, 1),
        ],
    ),
]
"""
    corpus_file.write_text(corpus_content, encoding="utf-8")
    cfg_file = tmp_path / "anti_patterns.json"

    harvested = harvest_corpus_anti_patterns(corpus_file, config_path=cfg_file)
    assert len(harvested) >= 5

    # Check key fixtures were extracted
    assert "sec_10k_p3_cash_flows" in harvested
    assert "Consolidated Statement of Cash Flows" in harvested
    assert "Cash generated by operating activities" in harvested
    assert "sec_10k_p7_balance_sheet" in harvested

    # Verify persisted in config
    loaded = load_banned_anti_patterns(cfg_file)
    assert "sec_10k_p3_cash_flows" in loaded
