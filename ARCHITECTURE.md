...
| :--- | :--- | :--- |
| **Specification Gaming** | Agent loosens or removes test assertions (`assert res == 5` $\to$ `assert res is not None`) to manufacture a passing run. | `GoldenBoundaryGate` blocks all commits touching `tests/golden/`, reinforced by `.github/CODEOWNERS` requiring human maintainer review. |
...
| **Context Window Exhaustion** | Agent spins across 20+ iterations repeatedly analyzing identical test failure traces. | External supervisor (`scripts/supervisor.py`) hashes error output and enforces a hard process abort at iteration 4. |
| **Dirty Repo State on Failure** | When an agent aborts at iteration 4, it leaves broken, uncommitted code changes across the tree. | `AgentSupervisor.rollback()` automatically executes an atomic rollback on failure and generates a clean `DIAGNOSTIC-REPORT.md`, eliminating human cleanup hours. |
| **Watcher Paradox (In-Band Tampering)** | Agent with shell access edits `scripts/verify_integrity.py` or runs `git commit --no-verify`. | Bifurcated gatekeeping: local hooks provide rapid feedback, while out-of-band GitHub Actions CI and `.github/CODEOWNERS` run in clean, isolated runners before code can merge. |
